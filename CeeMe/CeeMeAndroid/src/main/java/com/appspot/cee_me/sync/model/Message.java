/*
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License
 * is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing permissions and limitations under
 * the License.
 */
/*
 * This code was generated by https://code.google.com/p/google-apis-client-generator/
 * (build: 2013-10-30 15:57:41 UTC)
 * on 2013-11-19 at 21:30:37 UTC 
 * Modify at your own risk.
 */

package com.appspot.cee_me.sync.model;

/**
 * Model definition for Message.
 *
 * <p> This is the Java data model class that specifies how to parse/serialize into the JSON that is
 * transmitted over HTTP when working with the sync. For a detailed explanation see:
 * <a href="http://code.google.com/p/google-http-java-client/wiki/JSON">http://code.google.com/p/google-http-java-client/wiki/JSON</a>
 * </p>
 *
 * @author Google, Inc.
 */
@SuppressWarnings("javadoc")
public final class Message extends com.google.api.client.json.GenericJson {

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.Boolean accepted;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private DateTime creationDate;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private Device fromDevice;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private User fromUser;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private DateTime lastRetrievalDate;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private Media media;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String messageKey;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String openWithApp;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String openWithAppName;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String text;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private Device toDevice;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private User toUser;

  /**
   * The value may be {@code null}.
   */
  @com.google.api.client.util.Key
  private java.lang.String urlData;

  /**
   * @return value or {@code null} for none
   */
  public java.lang.Boolean getAccepted() {
    return accepted;
  }

  /**
   * @param accepted accepted or {@code null} for none
   */
  public Message setAccepted(java.lang.Boolean accepted) {
    this.accepted = accepted;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public DateTime getCreationDate() {
    return creationDate;
  }

  /**
   * @param creationDate creationDate or {@code null} for none
   */
  public Message setCreationDate(DateTime creationDate) {
    this.creationDate = creationDate;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public Device getFromDevice() {
    return fromDevice;
  }

  /**
   * @param fromDevice fromDevice or {@code null} for none
   */
  public Message setFromDevice(Device fromDevice) {
    this.fromDevice = fromDevice;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public User getFromUser() {
    return fromUser;
  }

  /**
   * @param fromUser fromUser or {@code null} for none
   */
  public Message setFromUser(User fromUser) {
    this.fromUser = fromUser;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public DateTime getLastRetrievalDate() {
    return lastRetrievalDate;
  }

  /**
   * @param lastRetrievalDate lastRetrievalDate or {@code null} for none
   */
  public Message setLastRetrievalDate(DateTime lastRetrievalDate) {
    this.lastRetrievalDate = lastRetrievalDate;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public Media getMedia() {
    return media;
  }

  /**
   * @param media media or {@code null} for none
   */
  public Message setMedia(Media media) {
    this.media = media;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public java.lang.String getMessageKey() {
    return messageKey;
  }

  /**
   * @param messageKey messageKey or {@code null} for none
   */
  public Message setMessageKey(java.lang.String messageKey) {
    this.messageKey = messageKey;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public java.lang.String getOpenWithApp() {
    return openWithApp;
  }

  /**
   * @param openWithApp openWithApp or {@code null} for none
   */
  public Message setOpenWithApp(java.lang.String openWithApp) {
    this.openWithApp = openWithApp;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public java.lang.String getOpenWithAppName() {
    return openWithAppName;
  }

  /**
   * @param openWithAppName openWithAppName or {@code null} for none
   */
  public Message setOpenWithAppName(java.lang.String openWithAppName) {
    this.openWithAppName = openWithAppName;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public java.lang.String getText() {
    return text;
  }

  /**
   * @param text text or {@code null} for none
   */
  public Message setText(java.lang.String text) {
    this.text = text;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public Device getToDevice() {
    return toDevice;
  }

  /**
   * @param toDevice toDevice or {@code null} for none
   */
  public Message setToDevice(Device toDevice) {
    this.toDevice = toDevice;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public User getToUser() {
    return toUser;
  }

  /**
   * @param toUser toUser or {@code null} for none
   */
  public Message setToUser(User toUser) {
    this.toUser = toUser;
    return this;
  }

  /**
   * @return value or {@code null} for none
   */
  public java.lang.String getUrlData() {
    return urlData;
  }

  /**
   * @param urlData urlData or {@code null} for none
   */
  public Message setUrlData(java.lang.String urlData) {
    this.urlData = urlData;
    return this;
  }

  @Override
  public Message set(String fieldName, Object value) {
    return (Message) super.set(fieldName, value);
  }

  @Override
  public Message clone() {
    return (Message) super.clone();
  }

}
